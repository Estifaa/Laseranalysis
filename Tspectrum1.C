...
void Tspectrum1()
{
  ...
  TString index_s;
  TString index_r;
  TString filename1 ="~/lmartin/agdaq_laser/ana/output02305.root"; 
  ...
  hdummy ->GetYaxis() ->SetRangeUser(0.,20000.);
  hdummy ->Draw();
  double maxpeak=0.;
  ...
  for(Int_t i=0; i<33; i++){
	...
	h1->Fill(x,y); 
      ...
      Int_t nfound = s->Search(h1,1,"",0.05);
      cout << "Found "<< nfound << " candidate peaks to fit." << endl;
	...
	par[3*p+4] = 2;
      ...
      f2->SetNpx(1000);
      //h1 ->Draw ();
      h1 ->SetStats(kFALSE); //gets rid of the box 
