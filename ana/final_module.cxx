...
public:
   ...
   TH2D* h_pad_amp_per_row = NULL;
   TH2D* h_pad_amp_per_row_cut[32];
   ...
   void BeginRun(TARunInfo* runinfo)
   {
      ...
      h_aw_map_time = new TH2D("h_aw_map_time", "aw hit time vs wire", NUM_AW, -0.5, NUM_AW-0.5, 500, 0, MAX_TIME);
      h_aw_map_amp  = new TH2D("h_aw_map_amp", "aw hit p.h. vs wire", NUM_AW, -0.5, NUM_AW-0.5, 1000, 0, MAX_AW_AMP);
      ...
      h_pad_amp_per_row = new TH2D("h_pad_amp_per_row", "pad hit amp per row; tpc row; amp", 8*4*18, -0.5, 8*4*18-0.5, 1000, 0,MAX_PAD_AMP );
     
      for (int i=0; i<32; i++) {
         char name[256];
         char title[256];
         sprintf(name, "h_pad_amp_per_row_cut_col%02d", i);
         sprintf(title, "pad hit amp per row col %d; tpc row; amp", i);
         h_pad_amp_per_row_cut[i] = new TH2D(name, title, 8*4*18, -0.5, 8*4*18-0.5, 1000, 0,MAX_PAD_AMP);
      }
   ...
   TAFlowEvent* AnalyzeFlowEvent(TARunInfo* runinfo, TAFlags* flags, TAFlowEvent* flow)
   {
      ...
      if (eph) {
         ...
         for (unsigned i=0; i<eph->fPadHits.size(); i++) {
            ...
            h_pad_amp_per_row->Fill(row, amp);
            if (time >5000 && time<6000) h_pad_amp_per_row_cut[col]->Fill(row, amp);
